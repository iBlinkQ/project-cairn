...
Cairn（路标石堆）是旅人用石头垒起的路标——名字本身就是这个 skill 想做的事：把学到的东西留在下一个项目找得到的地方。

Project Cairn 是一个 AI Agent Skill，适配 Claude Code、Codex 及其他兼容 Skill 的 Agent（OpenClaw、WorkBuddy……）。它做的是**经验知识化**：把 AI 协作项目里**亲身经历过的高价值经验**，自动沉淀成可复用的知识。踩坑经验、关键决策、探索成果、灵感洞见，都不会随会话结束而消失，而是传递给下一个项目。
...
一条经验的旅程：在 Agent 里做事 → 经 Project Cairn 沉淀 → 进知识库跨项目复用。
...
结论通过两条并行渠道汇入同一批 `cairn/<主题>.md`：

- **对话自发渠道** —— 与 Agent 协作过程中形成的结论（一个坑解决了、一个决策定了、一个值得留的发现），在形成的那一刻就写进对应主题文档；`LOG.md` 只留一行指针。
- **过程文档渠道** —— 项目本来就会产出的过程制品（[superpowers](https://github.com/obra/superpowers) 的 spec / plan、人工评审意见、会议讨论、实现复盘），等其中的结论稳定后再蒸馏过来。只取结论：过程文档本身留在原处，不搬进 `cairn/`。
...
- **人和 Agent 都能直接读写的格式。** 笔记就是 markdown + YAML frontmatter、frontmatter 至少带一个 `type` 字段，对齐 Google [Open Knowledge Format](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing)（OKF）的最小约定。门槛故意压得很低：手写可读可改、机器可识别、跨工具跨项目可交换。
- **每篇毕业笔记都带溯源。** `graduated_from` 记录从哪个项目、哪些源文件蒸馏而来，`contributors` 记录谁实质参与形成了这条知识，`graduated_by` 记录谁确认了毕业，`authoring_mode` 记录笔记本身怎么写成——四个彼此独立的问题，任何一条知识都查得到出处。
...
这套 provider 是可插拔的，不是写死的：满足 `references/provider-interface.md` 里那份行为契约的就是一个合法 provider——接一个新目标只需要写一个 adapter，规范本身不用动。
...
Project Cairn 以 Agent Skill 的形式分发——一条命令装进你的 AI 协作项目。
...
**其他兼容 Skill 的 Agent** —— clone 到该 Agent 读取 skill 的目录即可。
...
前置依赖：`git`；`scripts/*.sh` 需要 `bash`（macOS/Linux 原生支持，Windows 需要 WSL 或 Git Bash）；`scripts/*.py` 只需要 Python 3，不依赖 shell。目前还没有发包管理器版本，所以 clone 就是安装方式。
...
3. 当一件工作产生或依赖的结论本身够格毕业时，Agent 会先查再动手，而不是从零开始——先查本项目 `cairn/` 的主题文档，再查知识库的 `INDEX.md`，说不定这个坑早有人踩过。只有真正影响了产出的笔记，才在 `cairn/Cited.md` 记一条指针，绝不复制正文。这条反射写在 `AGENTS.md` 里，不需要额外命令；你也可以直接说*"查一下知识库里有没有关于 X 的结论"*。
4. 当你学到的东西对**另一个**项目也有用时，说一句*"把这个毕业到知识库"*。Agent 会提出候选，你确认范围，它写入配置好的 provider 并更新索引。
5. 定期地，或者感觉哪里不对劲的时候，说一句*"审查一下项目知识"*。Agent 会检查矛盾结论、过期结论、孤立笔记、缺失的毕业回指针，以及混入 `cairn/` 的工程资产。
...
| `references/upgrade.md` | 把已初始化的项目实例升级到当前 skill 规范，或检查它漂移了多少 |
...
- **[OKF（Open Knowledge Format）](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing)** 是知识文档的格式规范，不是竞争性的工作流。Project Cairn 的项目主题文档和知识库笔记都按 OKF 的最小约定组织——markdown + YAML frontmatter，至少一个 `type` 表明知识概念对象类别——这样笔记不会被锁死在某一个工具的形态里。Cairn 对齐的是这份最小约定，并非完整实现 OKF bundle。
